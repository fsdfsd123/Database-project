        ...
        ?>  	
      		  ...
      		  <div class =" xiaokuai">
                  <div><?php echo "<a href=fengbu.php?id=$rs[0]> $rs[1]</a>"?></div>
                    ...
                    <div><?php echo "<img class=biaoshitu src=image/".$rs[3]."/".$rs[4].".jpg> ";?> </div><br>
                      <?php 
                        $rate=mysql_query("SELECT avg(rating) as avgRating FROM `comment` where `company_ID` = '$feng';");
                        while($rating = mysql_fetch_array($rate))
                        {
...
<div id="pages" style="font-size:12px;"></div> <div id="current"></div>  
<script>
